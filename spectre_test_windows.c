...
void* KERNELBASE()
{
	...
	do
	{
		ModuleInfo = (PRTL_PROCESS_MODULES)VirtualAlloc(NULL, ReturnLen, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
		error = NtQuerySystemInformation(0x0B,  // SystemModuleInformation aka 11 here...
						 ModuleInfo, ReturnLen, &ReturnLen);

		if ((error == STATUS_INFO_LENGTH_MISMATCH)  && ModuleInfo)
		{
	...
	for (; error < ModuleInfo->NumberOfModules; ++error)
	{

		if (!strcmp((const char*)ModuleInfo->Modules[error].FullPathName + ModuleInfo->Modules[error].OffsetToFileName, "ntoskrnl.exe"))
		{
			KernelBase = ModuleInfo->Modules[error].ImageBase;
			break;
		}
